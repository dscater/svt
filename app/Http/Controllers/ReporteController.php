...
class ReporteController extends Controller
{
    ...
    public function r_inscritos(Request $request)
    {
        ...
        $inscripcions = Inscripcion::select(
            "inscripcions.*",
            "postulantes.paterno",
            DB::raw("DATE_FORMAT(r.created_at, '%d/%m/%Y %H:%i:%s') as requisitos_created_at")
        )
            ->join("postulantes", "postulantes.id", "=", "inscripcions.postulante_id")
            // solo juntar el requisito más reciente
            ->join(DB::raw("
        (
            SELECT requisitos.*
            FROM requisitos
            INNER JOIN (
                SELECT inscripcion_id, MAX(id) AS max_id
                FROM requisitos
                GROUP BY inscripcion_id
            ) latest ON latest.max_id = requisitos.id
        ) r
    "), "r.inscripcion_id", "=", "inscripcions.id");

        if ($unidad != 'todos') {
            $request->validate(['unidad' => 'required']);
            $inscripcions->where('unidad', $unidad);
        }
        ...
        $inscripcions = $inscripcions
            ...
            ->orderBy("postulantes.paterno", "ASC")
            ->get();
        ...
        if ($tipoR == 'pdf') {
            ...
            $pdf->AliasNbPages(); // Necesario para {nb}
            $pdf->SetMargins(1, 1);
            $pdf->AddPage("L", array(330, 216)); // oficio horizontal
            $pdf->SetFont('Arial', '', 10);
            ...
            // Encabezado de tabla
            $pdf->SetFont('Arial', 'B', $font_size);
            $pdf->CellUtf8(6, 7, 'N°', 1);
            $pdf->CellUtf8(16, 7, 'UNIDAD', 1);
            $pdf->CellUtf8(17, 7, 'CÓD. PREINSC.', 1);
            $pdf->CellUtf8(17, 7, 'CÓD. INSC.', 1);
            $pdf->CellUtf8(35, 7, 'NOMBRE POSTULANTE', 1);
            $pdf->CellUtf8(16, 7, 'C.I.', 1);
            $pdf->CellUtf8(5, 7, 'G', 1);
            $pdf->CellUtf8(16, 7, 'FECHA NAC.', 1);
            $pdf->CellUtf8(17, 7, 'CELULAR', 1);
            $pdf->CellUtf8($ancho, 7, 'CORREO', 1);
            $pdf->CellUtf8(16, 7, 'NRO. CUENTA', 1);
            $pdf->CellUtf8(34, 7, 'LUGAR PREINSCRIPCIÓN', 1);
            $pdf->CellUtf8(17, 7, 'SEDE', 1);
            $pdf->CellUtf8(28, 7, 'OBSERVACIÓN', 1);
            $pdf->CellUtf8(15, 7, 'ACCESO', 1);
            $pdf->CellUtf8(22, 7, 'FECHA INSC.', 1);
            ...
            foreach ($inscripcions as $registro) {
                $font_size = 6;
                $pdf->SetFont('Arial', '', $font_size);
                $pdf->CellUtf8(6, 6, $cont++, 1);
                $pdf->CellUtf8(16, 6, $registro->unidad, 1);
                $txt = $registro->codigoPre;
                while ($pdf->GetStringWidth($txt) > 17) {
                    $font_size--; // reducir tamaño
                    $pdf->SetFont('Arial', '', $font_size);
                }
                $pdf->CellUtf8(17, 6, $registro->codigoPre, 1);
                $txt = $registro->codigoInsc;
                while ($pdf->GetStringWidth($txt) > 17) {
                    $font_size--; // reducir tamaño
                    $pdf->SetFont('Arial', '', $font_size);
                }
                $pdf->CellUtf8(17, 6, $registro->codigoInsc, 1);
                ...
                $pdf->SetFont('Arial', '', $font_size);
                $pdf->CellUtf8(16, 6, $registro->postulante->full_ci, 1);
                $pdf->CellUtf8(5, 6, $registro->postulante->genero, 1);
                $pdf->CellUtf8(16, 6, $registro->postulante->fecha_nac_t, 1);
                $pdf->CellUtf8(17, 6, $registro->postulante->cel, 1);
                $txt = $registro->correo;
                while ($pdf->GetStringWidth($txt) > $ancho) {
                ...
                $pdf->CellUtf8(34, 6, $registro->lugar_preins, 1);
                $pdf->CellUtf8(17, 6, $registro->sede, 1);
                $txt = $registro->observacion;
                ...
                $pdf->CellUtf8(22, 6, $registro->requisitos_created_at, 1);
            ...
            $sheet->setCellValue('A' . $fila, 'N°');
            $sheet->setCellValue('B' . $fila, 'UNIDAD');
            $sheet->setCellValue('C' . $fila, 'CÓD. PREINSC.');
            $sheet->setCellValue('D' . $fila, 'CÓD. INSCRIPCIÓN');
            $sheet->setCellValue('E' . $fila, 'NOMBRE POSTULANTE');
            $sheet->setCellValue('F' . $fila, 'C.I.');
            $sheet->setCellValue('G' . $fila, 'GÉNERO');
            $sheet->setCellValue('H' . $fila, 'FECHA NAC.');
            $sheet->setCellValue('I' . $fila, 'CELULAR');
            $sheet->setCellValue('J' . $fila, 'CORREO');
            $sheet->setCellValue('K' . $fila, 'NRO. CUENTA');
            $sheet->setCellValue('L' . $fila, 'LUGAR PREINSCRIPCIÓN');
            $sheet->setCellValue('M' . $fila, 'SEDE');
            $sheet->setCellValue('N' . $fila, 'OBSERVACIÓN');
            $sheet->setCellValue('O' . $fila, 'ACCESO');
            $sheet->setCellValue('P' . $fila, 'FECHA DE INSCRIPCIÓN');
            $sheet->setCellValue('Q' . $fila, 'FECHA DE REGISTRO');
            ...
            foreach ($inscripcions as $key => $inscripcion) {
                $sheet->setCellValue('A' . $fila, $key + 1);
                $sheet->setCellValue('B' . $fila, $inscripcion->unidad);
                $sheet->setCellValue('C' . $fila, $inscripcion->codigoPre);
                $sheet->setCellValue('D' . $fila, $inscripcion->codigoInsc);
                $sheet->setCellValue('E' . $fila, $inscripcion->postulante->full_name);
                $sheet->setCellValue('F' . $fila, $inscripcion->postulante->full_ci);
                $sheet->setCellValue('G' . $fila, $inscripcion->postulante->genero);
                $sheet->setCellValue('H' . $fila, $inscripcion->postulante->fecha_nac_t);
                $sheet->setCellValue('I' . $fila, $inscripcion->postulante->cel);
                $sheet->setCellValue('J' . $fila, $inscripcion->correo);
                $sheet->setCellValue('K' . $fila, $inscripcion->postulante->nro_cuenta);
                $sheet->setCellValue('L' . $fila, $inscripcion->lugar_preins);
                $sheet->setCellValue('M' . $fila, $inscripcion->sede);
                $sheet->setCellValue('N' . $fila, $inscripcion->observacion);
                $sheet->setCellValue('O' . $fila, $inscripcion->user->acceso == 1 ? 'HABILITADO' : 'DENEGADO');
                $sheet->setCellValue('P' . $fila, $inscripcion->requisitos_created_at);
                $sheet->setCellValue('Q' . $fila, $inscripcion->fecha_registro_t);
